...
function AccessContext(context) {
  ...
  var principalModelName = context.principalModelName || undefined;

  if (principalId) {
    this.addPrincipal(principalType, principalId, principalName, principalModelName);
  }

  var token = this.accessToken || {};

  if (token.userId) {
    this.addPrincipal(Principal.USER, token.userId, null, token.userModelName);
  }
  if (token.appId) {
...
AccessContext.prototype.addPrincipal = function(principalType, principalId, principalName,
  principalModelName) {
  var principal = new Principal(principalType, principalId, principalName, principalModelName);
 ...
 * @class
 */
function Principal(type, id, name, modelName) {
  if (!(this instanceof Principal)) {
    return new Principal(type, id, name, modelName);
  }
  this.type = type;
  this.id = id;
  this.name = name;
